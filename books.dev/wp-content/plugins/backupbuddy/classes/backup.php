...
class pb_backupbuddy_backup {
	...
	public function deploy_start( $state ) {
		...
		pb_backupbuddy::status( 'details', 'Make importbuddy on remote destination.' );
		$newStep = array(
	...
	public function deploy_renderImportBuddy( $state ) {
		...
		if ( false === ( $response = $deploy->remoteAPI( 'renderImportBuddy' ) ) ) { 
		...
		$importbuddyPassword = md5( $state['apiKey'] ); // Double md5 like a rainbow.
		$importbuddyURL = $state['destination']['siteurl'] . '/importbuddy-' . $response['importFileSerial'] . '.php';
		pb_backupbuddy::status( 'details', 'Load importbuddy at `' . $importbuddyURL . '` with password `' . $importbuddyPassword . '`.' );
		
		
		// Load importbuddy in an iframe
		// Pass an importbuddy start state file
