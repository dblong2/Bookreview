...
class backupbuddy_core {
	...
	public static function backup_integrity_check( $file, $fileoptions = '', $options = array(), $skipLogRedirect = false ) {
		...
		if ( isset( $options['type'] ) && ( 'files' == $options['type'] ) ) {
			...
			if ( isset( $backup_options->options['table_sizes'] ) && ( count( $backup_options->options['table_sizes'] ) > 0 ) && ( $backup_options->options['data_version'] >= 1 ) ) { // BB v5.0+.
				...
				if ( ! isset( $backup_options->options['force_single_db_file'] ) ) {
					$backup_options->options['force_single_db_file'] = false;
				} else {
					if ( true === $backup_options->options['force_single_db_file'] ) {
				...
				if ( 'db' == $backup_options->options['type'] ) { // DB.
					...
					if ( pb_backupbuddy::$classes['zipbuddy']->file_exists( $file, 'db_1.sql' ) === true ) { // Commandline based.
						...
						if ( isset( $backup_options->options['breakout_tables'] ) && ( count( $backup_options->options['breakout_tables'] ) > 0 ) && ( true !== $backup_options->options['force_single_db_file'] ) ) { // Verify broken out table SQL files exist.
	 ...
	 */
	public static function send_remote_destination( $destination_id, $file, $trigger = '', $send_importbuddy = false, $delete_after = false, $identifier = '' ) {
	...
	public static function generate_api_key() {
		...
		$key = array(
			'key_version' => 1,
			'key_auth' => md5( $rand . pb_backupbuddy::$options['log_serial'] . $siteurl . $homeurl ),
			'siteurl' => $siteurl,
			'homeurl' => $homeurl,
			'key_created' => time(),
		);
	...
	public static function api_key_to_array( $key ) {
		$key = trim( $key );
