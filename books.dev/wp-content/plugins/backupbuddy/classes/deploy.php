...
class backupbuddy_deploy {
	...
	public function __construct( $api_key, $existingState = '' ) {
		...
		if ( is_array( $existingState ) ) { // User passed along an existing state to resume.
			...
			$this->_state = array(
				'apiKey' => $api_key,
				'destination' => backupbuddy_core::api_key_to_array( $api_key ),
				'startTime' => time(),
				'backupProfile' => '',
	...
	public function remoteAPI( $endpoint ) {
		$response = wp_remote_post( $this->_state['destination']['siteurl'], array(
			'method' => 'POST',
			'timeout' => 10,
			'redirection' => 5,
			'httpversion' => '1.0',
			...
			'headers' => array(),
			'body' => array(
					'backupbuddy_api_key' => $this->_state['apiKey'],
		...
		if ( is_wp_error( $response ) ) {
			return $this->_error( 'Error #8447755: ' . $response->get_error_message() . ' - URL: `' . $this->_state['destination']['siteurl'] . '`.' );
		} else {
